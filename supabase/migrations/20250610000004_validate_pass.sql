-- 808Tix MVP: atomic pass validation / check-in
-- Scanner identity comes from auth.uid() only — never from the client.
...
begin
  ...
  -- -------------------------------------------------------------------------
  -- Caller must own the event being scanned
  -- -------------------------------------------------------------------------
  select organizer_id
  ...
  -- -------------------------------------------------------------------------
  -- Active pass: atomic check-in (race-safe)
  -- -------------------------------------------------------------------------
  ...
  if found then
    ...
    -- Lost race or status changed between read and update
    select status
    into v_final_status
    from public.passes
    where id = v_pass.id;
