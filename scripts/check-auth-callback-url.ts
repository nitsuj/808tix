...
const hashSignup =
  'https://808tickets.com/#access_token=abc&refresh_token=def&expires_in=3600&token_type=bearer&type=signup';
...
const pkceUrl = 'https://808tickets.com/?code=auth-code-123';

assert(hasSupabaseAuthCallbackParams(parseAuthCallbackParams(pkceUrl)), 'detects PKCE code in query');

const tokenHashUrl = 'https://808tickets.com/?token_hash=th123&type=email';
...
assert(
  stripAuthCallbackFromUrl(hashSignup) === 'https://808tickets.com/',
  'strips hash tokens from URL',
);

assert(
  stripAuthCallbackFromUrl('https://808tickets.com/?code=abc&type=signup') ===
    'https://808tickets.com/',
  'strips query auth params',
);

const errorUrl = 'https://808tickets.com/#error=access_denied&error_description=Expired';

assert(getAuthCallbackIntent(parseAuthCallbackParams(errorUrl)).hasError, 'detects error callback');

assert(
  !hasSupabaseAuthCallbackParams(parseAuthCallbackParams('https://808tickets.com/')),
